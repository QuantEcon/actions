...
- **Actions (7)** — `setup-environment`, `build-lectures`, `build-jupyter-cache`, `restore-jupyter-cache`, `preview-netlify`, `preview-cloudflare`, `publish-gh-pages`; latest release **`v0.11.1`** (2026-08-07), and `@v0` points to it (both resolve to `a388639`)
...
| #106 docs sweep to the 2026.06 baseline | Addressed by the #106/#109 docs PR: baseline, both measured image sizes from the size job fixed in #172, the TeX Live wording, this table and CONTRIBUTING's `PLAN.md` row | Closes when that PR lands |
...
| #102 raw-GitHub 429 flake | Body superseded — the fix is CI-side retry, not the lecture-side change. #144 is open (retry, plus `lecture-python-programming` back in the matrix) | Review #144; validate by a maintainer dispatch after merge, since the workflow has no PR trigger |

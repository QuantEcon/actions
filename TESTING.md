...
| `deploy-cloudflare` | Everything short of a real deploy: the Access gate probe against a local stand-in for every response shape (exact team domain passes; wrong org, public `2xx`, same-host redirect, missing `Location`, `404`, `5xx`, unreachable, and a public asset behind a gated root all fail); the action refusing an ungated Worker **before wrangler is even installed**, asserted on the filesystem; every invalid input failing in validation; and the pinned wrangler accepting the generated config (`--dry-run` deploy and alias upload), which is what a Dependabot bump of the lockfile runs against. ❌ A real deploy needs a gated Worker and a token, so the first consumer deploy is its only proof |

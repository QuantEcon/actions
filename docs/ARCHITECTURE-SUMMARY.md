...
- **Images:** `ghcr.io/quantecon/quantecon:latest` (CPU) and `:gpu` (GPU)
- **Contents:** Ubuntu 24.04 + LaTeX + Miniconda + all Python dependencies
- **Build:** Weekly automated builds via GitHub Actions
...
- ✅ Container: LaTeX pre-installed, 20 seconds to pull and start
...
**Improvements:**
- ⚡ 3-4x faster (3-5 min vs 15-18 min)
...
3. ✅ Single global container (optimal simplicity)

**Rationale:**
- All lectures use same Python scientific stack
- Union of requirements is manageable (~100 packages)
- Disk space is cheap (2 GB acceptable)
- Massive reduction in complexity
- Easy to update (one PR affects all lectures)
